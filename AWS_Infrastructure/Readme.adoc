...
* Set the hostname of this EC2 instance. The hostname should be FQDN. [[hostname setting]]
...
* Verify the hostname of the IPA server with the below command. It should return the same FQDN set in the <<hostname setting>> step. 
[,shell]
----
cat /etc/hostname
----
