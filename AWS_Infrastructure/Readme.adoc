...
* Generate a password protected private key by using the below command. 
[,shell]
----
    ssh-keygen -t rsa  -f /root/.ssh/id_rsa_new
----

Create a password for this private key by entering it when prompted. 

image::/AWS_Infrastructure/images/centos_ver.png[]
